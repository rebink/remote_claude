...
export class MutagenController {
  ...
  private ensureSshWrapper(): string {
    ...
    // IdentitiesOnly=yes is critical: without it, SSH tries every default key
    // (~/.ssh/id_*) BEFORE the -i key we want. If the user has 5+ default
    // keys (common with ssh-agent), SSH hits MaxAuthTries=6 on the server
    // before our key is even attempted → "Too many authentication failures".
    const script =
      `#!/bin/sh\n` +
      `# Auto-generated by Remote Claude — injects per-project SSH config into mutagen's transport.\n` +
      `exec ssh ${keyFlag}${portFlag}-o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new -o BatchMode=yes "$@"\n`;
  ...
  async ensureSession(): Promise<{ ok: true } | { ok: false; error: string }> {
    ...
    if (this.sessionExists()) {
      ...
      const r = spawnSync('mutagen', ['sync', 'list', this.sessionName, '--template', '{{ .Status }}'], {
        encoding: 'utf8',
        timeout: 10000,
        env: this.mutagenEnv(),
      });
