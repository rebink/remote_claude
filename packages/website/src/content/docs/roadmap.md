...
- ✅ `patchwire` CLI with `setup`, `init`, `sync`, `ask`, `apply`, `doctor`
- ✅ `patchwire-agent` HTTP server with `/health` + `/ask`
- ✅ Bearer-token auth (constant-time)
- ✅ Selective per-file apply
- ✅ `patchwire-agent install/uninstall` (macOS launchd)
- ✅ Tailscale peer auto-discovery in `setup`
- ✅ 15 vitest tests + GitHub Actions CI on Node 20+22
- ✅ Installable via `pnpm add -g github:rebink/patchwire` (no npm publish required)
- ✅ Docs site

## Near-term (v0.2.x)

- 🔜 Publish to npm (`patchwire` registry slot)
...
- 🔜 `patchwire diff <last-N>` to inspect prior runs
...
- ❌ **Real-time co-editing.** Patchwire is request/response. If you want autocomplete or inline ghost-text, run Claude Code locally.
- ❌ **Becoming a full remote-IDE.** Editor stays local. Always.

## Want to influence the roadmap?

Open an issue at the [GitHub repo](https://github.com/rebink/patchwire/issues) and describe the workflow pain you're trying to solve. We weight requests by how concretely they're tied to a real workflow, not by upvote count.
